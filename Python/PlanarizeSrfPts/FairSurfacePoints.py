...
def SmoothN():
    ...
    while True:
        ...
        if mode == 1:
            ...
            FairPtsUV(Id, apts, Row , .1*factor, tanStyle, mode)
        elif mode == 0:
            ignore = 1
            ...
            if i > ignore-1 and i < Max - (ignore-1):
                ...
                FairPtsUV(Id, apts, [Idx] , .1*factor, tanStyle, mode)
        else:
...
def FairPtsUV(Id, apts, aIdx, factor, tanStyle, mode):
    ...
    #        d = rs.BrepPlaneClosestPoint(plane, apts[idx])[1]
    #        vec =apts[idx]-d
    #        vec_list.append(vec)
    #        l = l + vec.Length
    #        n = n + 1
    ...
    fc = Rhino.Geometry.Curve.Fair(curve,factor,.05,0,0,5)
    #ping = sc.doc.Objects.AddCurve(pc)
    
    """
    If using a plane:
    Find the grip closest point to the faired curve
    Find the intersections of the control polygon lines of the previous
    and next grips to the plane, moved to pass through the closest point.
    Take which ever int is closest and to the closest point and move the grip 
    along the chosen control polygon to be the same distace away, measured along 
    that line.
    Needs info about adjacent rows of points.
    
    """
    
    if fc:
        #sc.doc.Objects.AddCurve(fc)
