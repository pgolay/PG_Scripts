...
def get_meshpatch_inputs(meshId):
    
    """
    Set a plane to the mesh points.
    Get the bb corners on that plane- 
    Preview the plane, let the user drag the edges to the extents he likes:
        Add the lines and use GetObject/GetPt to move them
        Constrain GetPt to the plane axis
        Add dots at corners. Constrain rotation to plane Z at bb center
    let the user set degree and spans
    let the user rotate the plane(?)
    rotate from corner points, resize from edge lines
    preview with ghosted mesh?
    """
    
    mesh = sc.doc.Objects.Find(meshId).Geometry
    l = mesh.Geometry.Vertices.Count
    pts = [Rhino.Geometry.Point3d(mesh.Geometry.Vertices[n]) for n in range(l)]
    plane = Rhino.Geometry.Plane.FitPlaneToPoints(pts)[1]
    fitPlane  = Rhino.Geometry.Plane.FitPlaneToPoints(pts)[1]
    ...
    go.Objects(edgeLines)
    
    gp = GetPts(pts)
    gp.Constrain(Dir, True)
    ...
    i = 0
    while True :
        gp.SetCommandPrompt("Set edge locations.")
...
class MoveLines(Rhino.Input.Custom.GetPoint):
    
    def __init__(self, pts, lines, idx, base_point, type, ctr, vecDir):
        self.m_pts = pts
        self.m_lines = lines
        self.m_idx = idx
        self.m_xform = Transform.Identity
        self.m_point = base_point
        vecNow = e.CurrentPoint-base_point
    ...
    def CalculateRotation(self, point, vecDir,vecNow ctr):
        xform = RhinoTransform.Identity
        dir = point - self.m_point
        if (not dir.IsTiny()):
            xform = Transform.Rotation(vecDir, vecNow, ctr)
        return xform
        
    # OnMouseMove override
    def OnMouseMove(self, e):
        pNow = e.CurrentPoint
        self.m_xform = self.CalculateRotation(e.Point)
    ...
    def OnDynamicDraw(self, e):
        ...
        for i in range(l-1):
           if i != self.m_idx:
                e.Display.DrawLine(self.m_lines[i][0] + vec ,self.m_lines[i][1], + vec , LineColor, 2)
           else:
            e.Display.DrawLine( self.m_lines[i][0], self.m_lines[i][1], self.m_xform )
