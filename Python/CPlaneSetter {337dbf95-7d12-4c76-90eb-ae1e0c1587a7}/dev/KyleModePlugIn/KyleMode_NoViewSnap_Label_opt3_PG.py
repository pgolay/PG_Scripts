...
def kyle_mode_inputs():
    
    if scriptcontext.sticky.has_key("KyleAngle"):
        defAngle = scriptcontext.sticky["KyleAngle"]
    else: defAngle = 30
    
    if scriptcontext.sticky.has_key("blnLabels"):
        oldLabels = scriptcontext.sticky["blnLabels"]
    else: oldLabels = True
    
    if scriptcontext.sticky.has_key("blnNamed"):
        oldNamed = scriptcontext.sticky["blnNamed"]
    else: oldNamed = False
    
    optToggleLabels = Rhino.Input.Custom.OptionToggle(oldLabels, "Yes","No")
    optToggleNamed = Rhino.Input.Custom.OptionToggle(oldNamed, "No","Yes")
    optIntAngle = Rhino.Input.Custom.OptionInteger(defAngle, 1, 44)
    
    go = Rhino.Input.Custom.GetOption()
    go.AcceptNumber(True, False)
    go.AddOptionInteger("Angle", optIntAngle)
    go.AddOptionToggle("ShowLabels", optToggleLabels)
    go.AddOptionToggle("NamedCPlanes", optToggleNamed)
    go.SetCommandPrompt ("Set angle or choose option")
    
    while True:
        get_rc = go.Get()

        if go.CommandResult() != Rhino.Commands.Result.Success:
            ang = optIntAngle.CurrentValue
            scriptcontext.sticky["KyleAngle"] = dir
            blnNamed = optToggleNamed.CurrentValue
            scriptcontext.sticky["blnNamed"] = blnNamed
            blnLabels = optToggleLabels.CurrentValue
            scriptcontext.sticky["blnLabels"] = blnLabels
            return ang, blnNamed, blnLabels
            
        if get_rc.GotDefault(): 
            ang = optIntAngle.CurrentValue
            scriptcontext.sticky["KyleAngle"] = dir
            blnNamed = optToggleNamed.CurrentValue
            scriptcontext.sticky["blnNamed"] = blnNamed
        ...
        if get_rc == Rhino.Input.GetResult.Number:
            ang = go.Number()
            optIntAngle = Rhino.Input.Custom.OptionInteger(ang, 1, 44)
...
#        else: defAngle = 30
        ...
        inputs = kyle_mode_inputs()
        angle  = inputs[0]
        feedback = inputs[1]
        namedPlanes = inputs[2]
